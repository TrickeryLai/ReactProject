...
class Zoom extends React.Component{
    ...
    componentDidMount(){
        const layout=$("#layout-wrap"),//最外层容器
            ...
            picLeft=parseFloat(picBlock.offset().left),
            picTop=parseFloat(picBlock.offset().top),
            picWidth=initWidth,
        ...
        //根据设置的数值，设置对应的样式
        // 正常显示图片
        picBlock.css({width:this.props.initWidth||initWidth+'px',height:this.props.initHeight||initHeight+'px'});
        // 放大图片
        zoomBlock.css({width:this.props.initWidth*rate||initWidth*rate+'px',height:this.props.initHeight||initHeight*rate+'px'});
        // 鼠标对应区域
        cursorBlock.css({width:cursorInitWidth+'px',height:cursorInitHeight+'px'});
        // 放大显示区域
        zoomViewsBlock.css({[this.state.zoomPosition]:(-50-cursorInitWidth*rate)+'px',top:-50+'px',width:cursorInitWidth*rate+'px',height:cursorInitHeight*rate+'px'});
        ...
        return (
            <div className={this.props.className+" modules-zoom"} id={this.props.id} ref="moduleZoom">
                ...
                <div className="modules-zoom-picBlock" id="picBlock">
                    <div className="modules-zoom-pic" id="picBlock-img">
